...
from section2d.utils import EarlyStopping, WSELoss, MyDataset
from section2d.backbone import ResNet
...
dataset = MyDataset('./data/target.csv', [0,1],IM_PATH,IM_FORMAT,transform=train_im_trans, 
                        target_transform=target_trans, scaler=target_scaler)

num_trains = int(len(dataset) * 0.9)
num_tests = len(dataset) - num_trains
train_data, test_data = random_split(dataset, [num_trains, num_tests])
...
dataset = MyDataset('./data/target.csv', [0,2,3,4,5,6],IM_PATH,IM_FORMAT,transform=train_im_trans, 
                        target_transform=target_trans, scaler=target_scaler)
...
loss = WSELoss([0.3, 0.2, 0.2, 0.15, 0.15])
lr, num_epochs, batch_size = 0.001, 100, 64
...
dataset = MyDataset('./data/target.csv', [0,7,8,9,10,11],IM_PATH,IM_FORMAT,transform=train_im_trans, 
                        target_transform=target_trans, scaler=target_scaler)
